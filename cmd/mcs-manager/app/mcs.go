...
import (
	...
	"github.com/karmada-io/karmada/cmd/mcs-manager/app/options"
...
// NewMCSCommand creates a *cobra.Command object with default parameters
func NewMCSCommand(ctx context.Context) *cobra.Command {
	opts := options.NewOptions()

	cmd := &cobra.Command{
		Use:  "mcs-manager",
		Long: `The mcs-manager manages multi-cluster service discovery in a unified manner`,
	...
	cmd.AddCommand(sharedcommand.NewCmdVersion("mcs-manager"))
...
func run(ctx context.Context, opts *options.Options) error {
	...
	if err != nil {
		klog.Errorf("Failed to build mcs controller: %v", err)
		return err
	}
